...
def show_grading():
    ...
    if st.button("🚀 开始AI批改", type="primary", use_container_width=True):
        ...
        try:
            ...
            # 保存文件
            saved_files = save_files(all_uploaded_files, st.session_state.username)
            ...
            if result:
                ...
                with st.expander("📊 批改结果预览", expanded=True):
                    st.text_area("", value=result[:1000] + "..." if len(result) > 1000 else result, 
                ...
                with st.expander("🔍 调试信息", expanded=False):
                    ...
                    st.write("保存的文件：", len(saved_files) if 'saved_files' in locals() else "未定义")
        ...
        with st.spinner(""):
            try:
                ...
                if settings.get('has_marking_scheme') and task['marking_files']:
                    ...
                    # 读取文件内容
                    content_list = []
                    for file_path in task['answer_files']:
                        from functions.api_correcting.calling_api import process_file_content
                        content_type, content = process_file_content(file_path)
                        if content_type != 'error':
                            content_list.append(content)
                        else:
                            print(f"文件处理错误: {content}")
                    
                    result = enhanced_batch_correction_with_standard(
                        content_list,
                        file_info_list=task.get('all_file_info', []),
